...
export default async function EventDetailPage({ params }: { params: { id: string } }) {
  ...
  const otherMembers = event.members.filter((m) => m.userId !== userId)
    ...
    <div className="max-w-4xl mx-auto">
      ...
      <div className="card p-4 mb-8 flex items-center gap-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-warm-600 truncate">{inviteUrl}</p>
        </div>
          ...
          {otherMembers.length === 0 ? (
            <div className="card p-6 text-center">
              <p className="text-sm text-warm-400">
                No other members yet. Share the invite link above!
              </p>
            </div>
          ) : (
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <div>
                      ...
                      {member.role === 'ORGANIZER' && (
                        <span className="text-xs text-brand">Organizer</span>
                      )}
                    </div>
                  </div>
                  {member.attachedList ? (
                    ...
                    <p className="text-sm text-warm-400 italic">No list attached yet</p>
                  )}
                </div>
              ))}
            </div>
