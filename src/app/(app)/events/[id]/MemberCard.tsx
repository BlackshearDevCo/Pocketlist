          ...
          {list.items.length === 0 ? (
            ...
            <div className="space-y-4">
              ...
              {list.bundles.map((bundle) => {
                ...
                return (
                  <div key={bundle.id}>
                    <div className="flex items-center gap-2 bg-parchment rounded-t-xl border border-warm-200 px-3 py-2">
                      <svg className="w-3.5 h-3.5 text-brand flex-shrink-0" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                      ...
                      <ul className="divide-y divide-warm-100">
                        {bundleItems.map((item) => (
                          <li key={item.id} className="flex items-center gap-3 px-3 py-2.5">
                            ...
                            <div className="flex-1 min-w-0">
                              <p className="text-sm text-warm-700 break-words">{item.title}</p>
                              {item.price !== null && <p className="text-xs text-warm-400">${Number(item.price).toFixed(2)}</p>}
                              {item.notes && <p className="text-xs text-warm-400 mt-0.5 break-words">{item.notes}</p>}
                            </div>
                            {item.linkUrl && (
                              <a href={item.linkUrl} target="_blank" rel="noopener noreferrer"
                                className="text-brand hover:text-brand-hover flex-shrink-0 p-1" aria-label="View item">
                                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 6H5.25A2.25 2.25 0 003 8.25v10.5A2.25 2.25 0 005.25 21h10.5A2.25 2.25 0 0018 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25" />
                                </svg>
                              </a>
